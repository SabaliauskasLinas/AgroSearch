...
{
    public class FarolTractorScrapingService : IScrapingService
    {
        ...
        public string ServiceName => "*Farol Tractors";
        ...
        public void ScrapAsync(string query, string filters, int? costmin, int? costmax, int page = 1)
        {
            ...
            {
                if (page < 2)
                    _scrapBrowser.Navigate($"https://www.farol.co.uk/product-list/tractors/");
                if (!_rateLoaded)
                {
