...
{
    public class FarolForkliftsTelehandlersScrapingService : IScrapingService
    {
        ...
        public string ServiceName => "         *Forklits Telehandlers";
        ...
        public void ScrapAsync(string query, string synonyms, bool filtering, int? costmin, int? costmax, int page = 1)
        {
            ...
            {
                if (page < 2)
                    _scrapBrowser.Navigate($"https://www.farol.co.uk/product-list/telescopics-amp-plant/PVRLMO833551");
                if (!_rateLoaded)
                {
            ...
            try
            {
                ...
                foreach (HtmlElement add in ads)
                {
                    ...
                    var description = "Year: " + add.ElementsByClass("TD", "col_Year")?.FirstOrDefault()?.InnerText + "\n"
                        ...
                        + "Weight: " + add.ElementsByClass("TD", "col_Weigth")?.FirstOrDefault()?.InnerText + "\n"
                        + "Height: " + add.ElementsByClass("TD", "col_Height")?.FirstOrDefault()?.InnerText;
