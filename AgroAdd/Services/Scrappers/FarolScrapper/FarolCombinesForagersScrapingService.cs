...
{
    public class FarolCombinesForagersScrapingService : IScrapingService
    {
        ...
        public string ServiceName => "         *Combines Foragers";
        ...
        public void ScrapAsync(string query, string synonyms, bool filtering, int? costmin, int? costmax, int page = 1)
        {
            ...
            {
                if (page < 2)
                    _scrapBrowser.Navigate($"https://www.farol.co.uk/product-list/combines-amp-spfh/PVRLMO833551");
                if (!_rateLoaded)
                {
