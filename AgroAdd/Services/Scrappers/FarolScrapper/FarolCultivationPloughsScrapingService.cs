...
{
    public class FarolCultivationPloughsScrapingService : IScrapingService
    {
        ...
        public string ServiceName => "         *Cultivation Ploughs";
        ...
        public void ScrapAsync(string query, string synonyms, bool filtering, int? costmin, int? costmax, int page = 1)
        {
            ...
            {
                if (page < 2)
                    _scrapBrowser.Navigate($"https://www.farol.co.uk/product-list/cultivation-amp-ploughs/PVRLMO833551");
                if (!_rateLoaded)
                {
            ...
            try
            {
                ...
                foreach (HtmlElement add in ads)
                {
                    ...
                    var description = "Year: " + add.ElementsByClass("TD", "col_Year")?.FirstOrDefault()?.InnerText + "\n"
                        + "Width: " + add.ElementsByClass("TD", "col_Width")?.FirstOrDefault()?.InnerText;
